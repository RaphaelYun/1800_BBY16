...
function addCard(doc, parent) {
    ...
    let likeButton = newcard.querySelector('.like');
    if (likeButton) {
        likeButton.addEventListener("click", function() { addToFav(doc.id) });
    }
    let removeButton = newcard.querySelector('.remove');
    if (removeButton) {
        removeButton.addEventListener("click", function() { removeFav(doc.id) });
    }
...
function removeFav(foodId) {
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            console.log("User id: " + user.uid);
            var userDoc = db.collection("users").doc(user.uid);
            userDoc.update({
                favorites: firebase.firestore.FieldValue.arrayRemove(foodId)
            });
        }
    })
}
            ...
            db.collection("users").doc(user.uid)
                ...
                .then(function(doc) {
                    ...
                    favs.forEach(fav => { displayFavItem(fav) })
                        // favs.forEach(favs => { console.log("Favs " + favs) })
